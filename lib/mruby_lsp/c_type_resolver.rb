...
module MrubyLsp
  ...
  class CTypeResolver
    ...
    # `line` is the definition line addr2line reported beside the name; it
    # places a C++ function whose name clangd spells differently (#symbol_for).
    ...
    def compute_doc(file, func, line = nil)
      ...
      # anonymous one, and at file scope the name is not visible, so an appended
      # stub completes to nothing. After the definition it sits in the
      # function's own scope, whatever that is, and the same line serves C.
      lines = src.lines
